...
@hydra.main(
    config_path="../../../configs/eval", config_name="gen_ppl", version_base="1.3"
)
def main(cfg):
    ...
    eval_model = AutoModelForCausalLM.from_pretrained(
        cfg.pretrained_model, device_map="auto"
    )
    ...
    with torch.no_grad():
        for i in range(0, len(texts), cfg.batch_size):
            ...
            if cfg.first_chunk_only:
                ...
                eos_mask = (labels == eos_id).cumsum(
                    -1
                ) == 0  # valid until first EOS (exclusive)
