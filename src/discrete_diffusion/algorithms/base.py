...
class TrainerBase(L.LightningModule):
    ...
    def __init__(
        ...
        if (
            hasattr(self.config.noise, "_target_")
            and "HybridDiffusion" in self.config.noise._target_
        ):
            self.noise = hydra.utils.instantiate(
                self.config.noise, tokenizer=self.tokenizer
            )
        else:
        ...
        if config.neg_infinity_mode == "large-finite":
            ...
            raise ValueError(
                f"neg_infinity_mode must be 'large-finite' or 'true-inf', got '{config.neg_infinity_mode}'"
            )
    ...
    def _loss(self, x0, valid_tokens, current_accumulation_step=None, train_mode=False):
        ...
        return Loss(loss=token_nll, nlls=nlls, num_tokens=num_tokens)
    ...
    def on_train_epoch_end(self):
        ...
        if hasattr(self.metrics, "train_aux") and self.metrics.train_aux.weight > 0:
            self.log(
                name="train/aux",
                value=self.metrics.train_aux.compute(),
                on_step=False,
                on_epoch=True,
                sync_dist=True,
            )
