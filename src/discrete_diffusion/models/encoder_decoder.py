...
def get_sinusoidal_embedding(idxs, dim, base=10_000):
    device = idxs.device
    denominator = base ** (2 * torch.arange(dim // 2, device=device) / dim)
...
def _index_rotary(source_tensor, index):
    ...
    index = index.repeat(
        1, 1, source_tensor.shape[2], source_tensor.shape[3], source_tensor.shape[4]
    )
    # BS x L x 3 x 1 x H/2
...
class CrossAttnDDiTBlock(nn.Module):
    ...
    def forward(
        self, q_x, kv_x, t_cond, rotary_cos_sin_queries, rotary_cos_sin_keys, attn_mask
    ):
        bias_dropout_scale_fn = self._get_bias_dropout_scale()
        ...
        q = einops.rearrange(self.attn_q(q_x), "b s (h d) -> b s h d", h=self.n_heads)
        kv = einops.rearrange(
            self.attn_kv(kv_x), "b s (two h d) -> b s two h d", two=2, h=self.n_heads
        )
...
class Encoder(nn.Module):
    def __init__(self, n_blocks, dim, n_heads, cond_dim, mlp_ratio, dropout, adaLN):
        super().__init__()
        self.blocks = nn.ModuleList(
            [
                DDiTBlock(dim, n_heads, adaLN, cond_dim, mlp_ratio, dropout)
...
class GroupSwapLayer(nn.Module):
    ...
    def _prepare_query_processing(self):
        ...
        self.base_embedding = nn.Embedding(1, self.hidden_dim).weight

        if "freqs" in self.pre_query_mode:
            seq_pos = torch.arange(self.model_length)
            pos_freqs = get_sinusoidal_embedding(seq_pos, self.hidden_dim)
        ...
        if self.query_process_mode == "linear":
            self.query_processor = nn.Linear(self.hidden_dim, self.hidden_dim)
    ...
    def _compute_queries(
        self, x, positions, group_idxs, concrete_lengths, use_inference_mode
    ):
        ...
        if self.pre_query_mode.startswith("learn+freqs+"):
            ...
            if not use_inference_mode:
                ...
                queries = queries + torch.where(
                    mask_one[..., None],  # (bs, l, dim)
                    val_grp_zero[:, None],
                    val_grp_one[:, None],
                )
            else:
                ...
                extract_fn = (
                    _partition_logsumexp_inference
                    if "logsumexp" in self.pre_query_mode
    ...
    def forward(
        ...
        group_queries = self._compute_queries(
            x, position_queries, group_idxs, concrete_lengths, use_inference_mode
        )
        keys, values = self.hidden_to_key_value(x).split(self.hidden_dim, dim=-1)
        ...
        if self.pre_query_mode == "learn":
            ...
            group_queries = group_queries.repeat(1, expand_value, 1, 1)

        if self.pre_query_mode in ("learn", "learn+freqs") and not use_inference_mode:
            group_queries = group_queries.repeat(keys.shape[0], 1, 1, 1)
        group_queries = apply_rotary_pos_emb_single(
            group_queries, *rotary_cos_sin_queries
        )
        ...
        out = sdpa_attention_masked(
            group_queries, keys, values, cross_attn_mask, causal=False
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.group_swap = GroupSwapLayer(
            dim,
            n_heads,
            swap_pre_query_mode,
            swap_query_process_mode,
        ...
        self.layers = nn.ModuleList(
            [self._make_cross_attn_block() for _ in range(self.n_blocks)]
        )
    ...
    def forward(
        ...
        for layer in self.layers:
            if isinstance(layer, DDiTBlock):  # self attention
                x = layer(x, t_cond, rotary_cos_sin_queries, self_attn_mask)
...
class PartitionDIT(nn.Module):
    def __init__(self, config, vocab_size: int):
        ...
        self.output_layer = DDiTFinalLayer(
            dim, vocab_size, config.model.cond_dim, self.adaLN
        )
    ...
    def forward(
        ...
        rotary_cos_sin = self.rotary_emb(seq_len=self.model_length, device=x.device)
        if not use_inference_mode:  # Training
            ...
            self_attn_mask = make_inference_self_attn_mask(x.shape[1], concrete_lengths)
            rotary_cos_sin_queries = tuple(
                _index_rotary(vec, noisy_positions) for vec in rotary_cos_sin
            )
            rotary_cos_sin_keys = tuple(
                _index_rotary(vec, clean_positions) for vec in rotary_cos_sin
            )

        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            enc_out = self.encoder(x, t_cond, rotary_cos_sin_keys, self_attn_mask)
