...
class UDLM(trainer_base.Diffusion):
    ...
    def __init__(self, config, tokenizer):
        ...
        self._forward_process = (
            fp
            if isinstance(fp, UniformForwardProcess)
            else UniformForwardProcess(
                tokenizer=self.tokenizer, schedule=self.noise, name="uniform"
            )
        )
    ...
    def nll_per_token(self, log_x_theta, xt, x0, alpha_t, dalpha_t, low_var=False):
        ...
        alpha_t_prime = -1.0
        alpha_t = 1.0 - t[..., None]  # B, 1, 1
        x_bar = (
            vocab_size * alpha_t * F.one_hot(x0, self.vocab_size).float() + 1 - alpha_t
        )
        ...
        term2 = (
            (x_bar / x_bar_zt)
            * (x_bar_theta_zt.log() - x_bar_theta.log() + x_bar.log() - x_bar_zt.log())
