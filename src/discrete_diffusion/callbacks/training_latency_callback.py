...
class TrainingLatencyCallback(L.Callback):
    ...
    def _time_forward(
        self, pl_module, vocab_size, batch_size, sequence_length, device
    ) -> _TimingStats:
        latencies, throughputs = self._measure(
            pl_module, vocab_size, batch_size, sequence_length, device, backward=False
        )
    ...
    def _time_forward_backward(
        ...
        latencies, throughputs = self._measure(
            pl_module, vocab_size, batch_size, sequence_length, device, backward=True
        )
        return self._aggregate(latencies, throughputs)

    def _measure(
        self, pl_module, vocab_size, batch_size, sequence_length, device, backward
    ):
        ...
        for _ in range(total_iters):
            x0 = torch.randint(
                0, vocab_size, size=(batch_size, sequence_length), device=device
            )
    ...
    def _log_results(self, trainer, results):
        if trainer.logger is not None:
            metrics = {
                ...
                "throughput/forward_mean_tokens_per_s": results["forward"][
                    "throughput_mean"
                ],
                "throughput/forward_std_tokens_per_s": results["forward"][
                    "throughput_std"
                ],
                "latency/forward_backward_mean_s": results["forward_backward"][
                    "latency_mean"
                ],
                "latency/forward_backward_std_s": results["forward_backward"][
                    "latency_std"
                ],
                "throughput/forward_backward_mean_tokens_per_s": results[
                    "forward_backward"
                ]["throughput_mean"],
                "throughput/forward_backward_std_tokens_per_s": results[
                    "forward_backward"
                ]["throughput_std"],
            }
