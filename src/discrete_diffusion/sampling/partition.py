...
class PartitionSampler(Sampler):
    ...
    def compute_posterior(
        self, model, x, t, dt, p_x0=None, group_idxs=None, noise_removal_step=False
    ):
        ...
        if p_x0 is None:
            log_p_x0 = model.forward(
                x, model._sigma_from_alphat(alpha_t), group_idxs=group_idxs
            )
        ...
        if group_idxs is not None:
            ...
            group_idxs = torch.where(
                out == x, group_idxs, torch.logical_not(group_idxs)
            )
    ...
    def compute_posterior_efficient(
        self, model, x, t, dt, p_x0, clean_positions, noisy_positions, concrete_lengths
    ):
        ...
        for i in range(num_steps):
            ...
            if predictor == "ddpm":
                ...
                p_x0_cache, x_next, group_idxs = self.compute_posterior(
                    model=model, x=x, t=t, dt=dt, p_x0=p_x0_cache, group_idxs=group_idxs
                )
    ...
    @torch.no_grad()
    def generate_efficient_uniform(
        ...
        clean_positions = torch.zeros(
            size=(num_samples, 1), device=model.device, dtype=torch.int64
        )
        noisy_positions = torch.arange(
            ...
            device=model.device,
            dtype=torch.int64,
        )[None].repeat(num_samples, 1)
        # Random permutation
        rand = torch.rand_like(noisy_positions, dtype=torch.float32)
        shuffled_indices = rand.argsort(dim=-1)
        noisy_positions = torch.gather(noisy_positions, dim=-1, index=shuffled_indices)
        concrete_lengths = torch.ones(
            size=(num_samples,), device=model.device, dtype=torch.int64
        )

        if self.config.model.length % num_steps != 0:
            raise ValueError(
                f"Length {self.config.model.length} must be divisible by steps {num_steps}"
            )

        n_tok_per_normal_step = self.config.model.length // num_steps
        all_n_tok_per_step = torch.full(
            size=(num_steps,), fill_value=n_tok_per_normal_step
        )
        # Last step might need more tokens
        all_n_tok_per_step[-1] += (
            self.config.model.length - num_steps * n_tok_per_normal_step
        )
    ...
    def _gen_eff_non_unif_post_process(
        self,
        x,
        concrete_lengths,
        n_denoise_per_seq,
        denoised_token_values,
        clean_positions,
        noisy_positions,
        noisy_pos_input,
    ):
        ...
        if n_tok_to_add > 0:
            pad = torch.zeros(
                size=(x.shape[0], n_tok_to_add), dtype=x.dtype, device=x.device
            )
        ...
        for i in range(x.shape[0]):
            if n_denoise_per_seq[i] == 0:
                continue
            x[i, concrete_lengths[i] : new_concrete_lengths[i]] = denoised_token_values[
                i, : n_denoise_per_seq[i]
            ]
    ...
    @torch.no_grad()
    def generate_efficient_non_uniform(
        self, model, *, num_samples, num_steps, eps, inject_bos
    ):
        ...
        for t in timesteps[:-1]:
            t = t * torch.ones(x.shape[0], 1, device=model.device)
            bin_count = torch.ones(size=(num_samples,), device=prob_denoise.device)
            bin_count *= self.config.model.length
            n_denoise_per_seq = torch.binomial(count=bin_count, prob=prob_denoise).to(
                int
            )
            n_denoise_per_seq = torch.min(
            ...
            (x, clean_positions, concrete_lengths) = (
                self._gen_eff_non_unif_post_process(
                    x,
                    concrete_lengths,
                    n_denoise_per_seq,
                    denoised_token_values,
        ...
        if not torch.all(concrete_lengths == self.config.model.length):
            n_denoise_per_seq = self.config.model.length - concrete_lengths
            noisy_pos_input = noisy_positions[
                :, : self.config.model.length - concrete_lengths.min()
            ]
            denoised_token_values = self.compute_posterior_efficient(
                model=model,
                x=x,
