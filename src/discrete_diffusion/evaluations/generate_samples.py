...
def main(cfg):
    ...
    if "hyper_parameters" not in ckpt:
        raise ValueError(
            "Checkpoint does not contain 'hyper_parameters'. Cannot load config."
        )
    ...
    print(
        f"Generating {num_samples} samples (batch_size={batch_size}, steps={num_steps or 'default'})"
    )
    ...
    with tqdm.tqdm(total=num_samples, desc="Sampling", dynamic_ncols=True) as pbar:
        for i in range(0, num_samples, batch_size):
            ...
            samples = model.generate_samples(
                num_samples=current_batch_size, num_steps=num_steps
            )
