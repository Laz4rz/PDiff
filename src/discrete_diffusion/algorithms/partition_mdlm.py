...
class PartitionMDLM(MDLM):
    ...
    def _validate_configuration(self):
        assert not self.time_conditioning, "Partition MDLM cannot be time conditioned."

        assert self.sampling_mode in {
            "naive",
            "efficient-uniform",
            "efficient-non-uniform",
        }, self.sampling_mode
    ...
    def nll(self, x0, current_accumulation_step=None, train_mode=False):
        ...
        log_x_theta = self.forward(x0, torch.zeros_like(sigma), group_idxs)
        ...
        nll_alpha_t = torch.where(group_idxs != 0, alpha_t, alpha_t_complement)
        nll_dalpha_t = torch.where(group_idxs != 0, dalpha_t, dalpha_t_complement)
        ...
        if train_mode:
            ...
            return torch.where(group_idxs == 1, tokens_nll, 0.0)
