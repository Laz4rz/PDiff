...
def find_delimiter_positions(
    tokens: Sequence[int], delimiter_tokens: Sequence[int]
) -> List[int]:
...
def recursive_split(
    tokens: Sequence[int], max_length: int, delimiter_tokens: Sequence[int]
) -> List[List[int]]:
    ...
    if not candidates:
        return [
            list(tokens[i : i + max_length]) for i in range(0, len(tokens), max_length)
        ]
    ...
    right = recursive_split(
        tokens[split_point + delimiter_len :], max_length, delimiter_tokens
    )
    return left + right
...
def chunk_documents(
    ...
    *,
    ...
    if pad_token is None:
        raise ValueError(
            "Tokenizer must define a pad_token when using chunk_documents()."
        )
    ...
    for text in texts:
        ...
        for chunk in chunks:
            if len(chunk) > max_length:
                raise ValueError(
                    "Chunk size exceeded max_length; recursive_split should prevent this."
                )
