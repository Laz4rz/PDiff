...
class AbsorbingSampler(Sampler):
    ...
    def compute_posterior(self, model, x, t, dt, p_x0=None, noise_removal_step=False):
        ...
        if p_x0 is None:
            log_p_x0 = model.forward(x, model._sigma_from_alphat(alpha_t))
        ...
        should_denoise_draw = (
            torch.rand_like(x, dtype=torch.float64, device=x.device) < prob_denoise
        )
    ...
    def generate(self, model, *, num_samples, num_steps, eps, inject_bos):
        ...
        inject_bos = (
            self.config.sampling.inject_bos if inject_bos is None else inject_bos
        )
        if inject_bos:
        ...
        for i in range(num_steps):
            ...
            if predictor == "ddpm":
                _, x = self.compute_posterior(model=model, x=x, t=t, dt=dt, p_x0=None)
        ...
        _, x = self.compute_posterior(
            model=model, x=x, t=t0, dt=None, p_x0=p_x0_cache, noise_removal_step=True
        )
