...
def requests_to_dataset(config, requests, tokenizer, num_proc):
    def _tokenize(e):
        ...
        prefix_tokens = tokenizer(
            e["prefix"], return_attention_mask=False, add_special_tokens=False
        )["input_ids"]
        target_tokens = tokenizer(
            e["target"], return_attention_mask=False, add_special_tokens=False
        )["input_ids"]
    ...
    seq_lenths = [len(x["prefix"]) + len(x["target"]) for x in ds]

    num_larger = len([x for x in seq_lenths if x > config.model.length])
    if num_larger > 0:
        print(
            f"\033[91mThere are some examples that are longer "
...
def _eval_suffix_nll_generators(
    config, module, prefix, suffix, batch_size, num_samples
):
    device = module.device
    assert num_samples % batch_size == 0
    full_sentence = (
        torch.cat([prefix, suffix], dim=-1).repeat(batch_size, 1).to(module.device)
    )
...
def eval_suffix_nll(config, module, prefix, suffix, batch_size, num_samples):
    all_losses = []
    generator = _eval_suffix_nll_generators(
        config, module, prefix, suffix, batch_size, num_samples
    )
    for xt, x0, group_idxs, sigma, alpha_t, dalpha_t in generator:
        ...
        token_nll = module.nll_per_token(log_x_theta, xt, x0, alpha_t, dalpha_t)
        if group_idxs is not None:
...
class MGMEvalWrapper(LM):
    ...
    def suffix_greedy_prediction(self, prefix, target):
        if self.config.algo.name == "mdlm":
            return self._suffix_greedy_prediction_mdlm(prefix, target)
        elif self.config.algo.name == "partition-mdlm":
            return self._suffix_greedy_prediction_pgm(prefix, target)
        else:
    ...
    def _suffix_greedy_prediction_mdlm(self, prefix, target):
        ...
        noisy_target = torch.tensor(noisy_target, device=self.device)
        prefix = prefix.to(self.device)
        seq = torch.concatenate([prefix, noisy_target], dim=-1).reshape(1, -1)
        sigma = torch.zeros(size=(seq.shape[0], 1), device=self.device)
    ...
    def _suffix_greedy_prediction_pgm(self, prefix, target):
        ...
        group_idxs = torch.tensor(
            [0] * len(prefix) + [1] * (len(target) - 1) + [0], device=self.device
        )[None]
        sigma = torch.zeros(size=(seq.shape[0], 1), device=self.device)
    ...
    def loglikelihood(self, requests: list[Instance]) -> list[tuple[float, bool]]:
        dataset = requests_to_dataset(
            self.config, requests, self.tokenizer, self.num_proc
        )
