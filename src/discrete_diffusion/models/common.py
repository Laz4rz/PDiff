...
try:  # flash-attn is optional but recommended
    ...
    ImportError,
    RuntimeError,
):  # pragma: no cover - flash_attn required in production
...
def split_and_apply_rotary_pos_emb(
    qkv: torch.Tensor, rotary_cos_sin: typing.Tuple[torch.Tensor, torch.Tensor]
):
...
def apply_rotary_pos_emb_torchscript(
...
def apply_rotary_pos_emb_single(
    vec: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor
):
    """Apply rotary to a single tensor (q or k) with shape (B, S, H, D)."""
    if flash_attn is None:
        raise RuntimeError(
            "flash_attn is required for rotary single-vector application"
        )
    with torch.amp.autocast("cuda", enabled=False):
...
def flash_varlen_attention_qkvpacked(
    ...
    if flash_attn is None:
        raise RuntimeError(
            "flash_attn is not available for flash_varlen_attention_qkvpacked"
        )
