...
class MDLM(trainer_base.AbsorbingState):
    ...
    def _process_model_output(self, model_output, xt, sigma):
        index = torch.full(
            (xt.shape[0], xt.shape[1], 1), self.mask_id, device=xt.device
        )
        model_output = torch.scatter(model_output, -1, index, self.neg_infinity)
        model_output = torch.where(
            (xt != self.mask_id)[..., None], self.neg_infinity, model_output
        )
    ...
    def nll_per_token(self, log_x_theta, xt, x0, alpha_t, dalpha_t, low_var=False):
        ce_loss = liger_cross_entropy(
            log_x_theta.flatten(0, 1), x0.flatten(0, 1), reduction="none"
        ).view_as(x0)
